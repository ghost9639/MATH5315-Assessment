...
For simplicity, we split the data assuming the crisis happened overnight at the start of August 2007. Firstly, direct residual testing finds no problems with the distribution of the residuals for a basic ARMA(0, 1) model on both datasets (Appendix \ref{sec:org3bef462}), but the residuals stop following a white noise process in the post-crash dataset (Appendix \ref{sec:org2019b16}). This could be due to stochastically changing variance, as ARIMA models are not designed around conditional variance. However, this does suggest that the mean can effectively be modelled by an ARMA distribution. We can now fit the GARCH models to the pre-crisis and post-crisis datasets in order to quantify changes to volatility in the time period. Looking at the models (Appendix \ref{sec:org3bef462}), we can see that they satisfy key model assumptions. GARCH models generally assume that:
\begin{enumerate}
...
\label{sec:orga3c4d6e}
\begin{lstlisting}[language=r,label=lst:org95e570b,caption={Question 1: Visualisation of hard drive lifetimes},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org9108221,caption={Question 1: fitted model},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org0f623aa,caption={Question 1: Simulated models},captionpos=b,numbers=none]
set.seed(1234)
...
\label{sec:orgbaba261}
\begin{lstlisting}[language=r,label=lst:orge9711ed,caption={Question 1: fitting gamma and exponential distributions},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org5cd8758,caption={Question 1: Kolmogorov-Smirnov tests},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org74350aa,caption={AIC and BIC information for models},captionpos=b,numbers=none]
n <- dim(hard_drives) 
...
\label{sec:org46626dd}
\begin{lstlisting}[language=r,label=lst:org64127f7,caption={Time series plotting of European Indices data},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org18ff1d1,caption={Visualisation of ACFs and PACFs for DAX closing price},captionpos=b,numbers=none]
par(mfrow = c(2,1))
...
\begin{lstlisting}[language=r,label=lst:org605286c,caption={Predictions for DAX index from ARIMA model},captionpos=b,numbers=none]
Q2_arima_proj <- forecast(mod_arima_DAX, h = 10)

autoplot(Q2_arima_proj, ylab = "DAX20 Closing Price")
\end{lstlisting}
\section{Question 2 Fitting}
\label{sec:orgae590e6}
\begin{lstlisting}[language=r,caption={Stationarity testing DAX prices and first difference},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org8b74f89,caption={ARIMA fit on differenced DAX data},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:orgfb9f50d,caption={Question 2: Post-regression statistics for ARIMA model},captionpos=b,numbers=none]
...
\label{sec:org1cd9598}
\begin{lstlisting}[language=r,label=lst:org5184d3f,caption={Stationarity and Cointegration tests},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:orgde77e44,caption={Question 2 VAR model initialisation},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org0370fd4,caption={Question 2: VAR Forecasting},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org836a4a6,caption={Question 2: VAR post-regression statistics},captionpos=b,numbers=none]
## serial correlation
...
\label{sec:org8782d5f}
\begin{lstlisting}[language=r,label=lst:org4f98168,caption={Question 3: Visualisation of deaths and ACFs},captionpos=b,numbers=none]
...
\caption{\label{fig:org1eb72b2}Question 3: Visualisation of deaths and ACFs}
\end{figure}

\begin{lstlisting}[language=r,label=lst:org5d207a2,caption={Question 3: Visualisation of change in deaths and ACFs},captionpos=b,numbers=none]
par(mfrow = c(3,1))
...
\begin{lstlisting}[language=r,label=lst:orgd4aa3ba,caption={Question 3: Projections of deaths using base ARIMA model},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org1875317,caption={Question 3: Projections of deaths using backfitted ARIMA model},captionpos=b,numbers=none]
sqdeaths <- deaths
...
\begin{lstlisting}[language=r,label=lst:org7a97f27,caption={Question 3: Visual residuals testing for base model},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org1aea790,caption={Question 3: Visual residuals testing for back-fitted model},captionpos=b,numbers=none]
temp_mod_sqd <- arima(sqdeaths$wk.deaths, c(2,1,2))

ts.diag(temp_mod_sqd)
\end{lstlisting}

\begin{lstlisting}[language=r,label=lst:org5989d79,caption={Question 3: Model values},captionpos=b,numbers=none]
death_mod
mod_sqd
\end{lstlisting}
\section{Question 3 Fitting}
\label{sec:orgae228c6}
\begin{lstlisting}[language=r,label=lst:org67d242e,caption={Question 3: Stationarity testing},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:orgf5d1ea2,caption={Question 3: Post-regression residual testing},captionpos=b,numbers=none]
...
\label{sec:org2019b16}
\begin{lstlisting}[language=r,label=lst:org85d834e,caption={Question 4: Logged returns visualised},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org0126790,caption={Visual residuals test for ARIMA model},captionpos=b,numbers=none]
...
\caption{\label{fig:orgef1388e}Visual residuals test for ARIMA model}
\end{figure}
\section{Question 4 Fitting}
\label{sec:org3bef462}
\begin{lstlisting}[language=r,label=lst:org223154b,caption={Question 4: Stationarity testing},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org63f9454,caption={Pre-Post Crisis split},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:orgf80e3cd,caption={Question 4: ARIMA mean model fitting},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org1129c17,caption={Question 4: ARCH test},captionpos=b,numbers=none]
...
\begin{lstlisting}[language=r,label=lst:org7f2cef7,caption={Question 4: GARCH Model Fitting},captionpos=b,numbers=none]
...
Coefficient(s):
 ...
 3.5594e-04  -9.9023e-01   9.9098e-01   7.8292e-03   2.5224e-07   4.6400e-02  
      beta1  
 9.4962e-01  
...
Description:
 Tue Dec 30 17:28:04 2025 by user:  
...
-4.9340e-05   1.9265e-01  -2.5909e-02  -1.8975e-01   2.9106e-07   4.4219e-02  
      beta1  
 9.4923e-01  

#+TITLE: MATH5306 Assessment
#+AUTHOR: 201605543
#+OPTIONS: toc:nil prop:nil
#+LATEX_CLASS: article
...
#+LATEX_HEADER: \lstset{backgroundcolor=\color{gray!10},basicstyle=\ttfamily\small,breaklines=true,showspaces=false, showstringspaces=false}
#+latex_header: \renewcommand{\topfraction}{0.85}
#+latex_header: \renewcommand{\bottomfraction}{0.7}
...
#+caption: Visual residuals testing for the base ARIMA model
#+attr_latex: :float t :width 0.8\linewidth :height 0.4\textheight :keepaspectratio
#+RESULTS: Q3DeathTests
[[file:babelTSDiagDeath.pdf]]

#+caption: Visual residuals testing for the back-fitted ARIMA model
#+attr_latex: :float t :width 0.8\linewidth :height 0.4\textheight :keepaspectratio
#+RESULTS: Q3SQDeathTests
[[file:babelTSDiagSQDeath.pdf]]

* Question 4
We are given a univariate time series of exchange rates between the US Dollar and British Pound over 12 years. We want to understand changes in volatility before and after the credit crisis. If we want to stochastically model volatility, we can use the GARCH model. This model expects returns, so we can take the logged returns (Appendix [[#app:Q4Viz]]). Looking at Figure 13, the data visually looks stationary. We confirm this using a augmented Dickey-Fuller test in Appendix [[#app:Q4Fit]]. 

#+attr_latex: :float t :width 0.8\linewidth :height 0.4\textheight :keepaspectratio
#+caption: Logged returns for USD/GBP exchange rate
#+RESULTS: Q4RetViz
[[file:babelEXRetTSPLT.pdf]]

#+RESULTS: Q4DateSplit
: [1] "The pre-crash data ranges from 2000-05-02 to 2007-07-31, and"
: [1] "the post-crash data ranges from 2007-08-01 to 2012-04-30."

For simplicity, we split the data assuming the crisis happened overnight at the start of August 2007. Firstly, direct residual testing finds no problems with the distribution of the residuals for a basic ARMA(0, 1) model on both datasets (Appendix [[#app:Q4Fit]]), but the residuals stop following a white noise process in the post-crash dataset (Appendix [[#app:Q4Viz]]). This could be due to stochastically changing variance, as ARIMA models are not designed around conditional variance. However, this does suggest that the mean can effectively be modelled by an ARMA distribution. We can now fit the GARCH models to the pre-crisis and post-crisis datasets in order to quantify changes to volatility in the time period. Looking at the models (Appendix [[#app:Q4Fit]]), we can see that they satisfy key model assumptions. GARCH models generally assume that:
1) The average value is already modelled / a white noise process 
2) Stationarity, finite variance
3) Presence of ARCH prior to modelling
...
* Question 1 Visualisations
:PROPERTIES:
:CUSTOM_ID: app:Q1Viz1
:END:

#+name: babBatDistViz
#+caption: Question 1: Visualisation of hard drive lifetimes
#+begin_src R :session MyR :results graphics file :file babelBatteryDist.pdf :exports both
...
#+name: babQ1VizFit
#+caption: Question 1: fitted model
#+begin_src R :session MyR :results graphics file :file babelHardDriveLifeFit.pdf :exports both
...
#+name: babQ1Sim
#+caption: Question 1: Simulated models
#+begin_src R :session MyR :results graphics file :file babelQ1Simulations.pdf :exports both
set.seed(1234)
...
#+name: Q1Fitting
#+caption: Question 1: fitting gamma and exponential distributions
...
#+caption: Fitted model statistics
#+RESULTS: Q1Fitting
...
#+name: Q1KStest
#+caption: Question 1: Kolmogorov-Smirnov tests
#+begin_src R :session MyR :results output :exports both
...
#+name: Q1IC
#+caption: AIC and BIC information for models
#+begin_src R :session MyR :results value table :colnames yes :exports both
n <- dim(hard_drives) 
...
* Question 2 Visualisations
:PROPERTIES:
:CUSTOM_ID: app:Q2Viz
:END:

#+name: eurotsplt
#+caption: Time series plotting of European Indices data
#+begin_src R :session MyR :results graphics file :file babelStockClosePlt.pdf :exports both
...
#+name: DAXACFs
#+caption: Visualisation of ACFs and PACFs for DAX closing price
#+begin_src R :session MyR :results graphics file :file babelACFPACFDAX.pdf :exports both
  par(mfrow = c(2,1))
...
#+name: Q2ARIMAPred
#+caption: Predictions for DAX index from ARIMA model
#+begin_src R :session MyR :results graphics file :file babelProjDAX20.pdf :exports both
...
#+caption: Stationarity testing DAX prices and first difference
#+begin_src R :session MyR :results output :exports both
...
#+name: Q2ARIMA
#+caption: ARIMA fit on differenced DAX data
#+begin_src R :session MyR :results output :exports both
...
#+name: Q2ARIMAstats
#+caption: Question 2: Post-regression statistics for ARIMA model
#+begin_src R :session MyR :results output :exports both
...
* Question 2 VAR model
:PROPERTIES:
:CUSTOM_ID: app:var
:END:

#+name: Q2StationarityandCointegration
#+caption: Stationarity and Cointegration tests
#+begin_src R :session MyR :results value table :colnames yes :exports both
...
#+name: Q2VarDec
#+caption: Question 2 VAR model initialisation
#+begin_src R :session MyR :results output :exports both
...
#+caption: VAR model specifications
#+RESULTS: Q2VarDec
#+begin_example
$selection
...
#+name: Q2IRFTabs 
#+caption: Question 2: VAR Forecasting
#+begin_src R :session MyR :results graphics file :file babelEuroVAR.pdf :exports both
...
#+name: Q2VARstats
#+caption: Question 2: VAR post-regression statistics
#+begin_src R :session MyR :results output :exports both
## serial correlation
...
#+name: Q3DeathsViz
#+caption: Question 3: Visualisation of deaths and ACFs
#+begin_src R :session MyR :results graphics file :file babelDeathPlot.pdf :exports both
...
#+attr_latex: :float t :width 0.8\linewidth :height 0.4\textheight :keepaspectratio
#+RESULTS: Q3DeathsViz
[[file:babelDeathPlot.pdf]]

#+name: Q3DDeathsViz
#+caption: Question 3: Visualisation of change in deaths and ACFs
#+begin_src R :session MyR :results graphics file :file babelDDeathsPlt.pdf :exports both
par(mfrow = c(3,1))
...
#+name: Q3DeathProj
#+caption: Question 3: Projections of deaths using base ARIMA model
#+begin_src R :session MyR :results graphics file :file babelDeathsProjections.pdf :exports both
...
#+name: Q3SQDeathProj
#+caption: Question 3: Projections of deaths using backfitted ARIMA model
#+begin_src R :session MyR :results graphics file :file babelSQDeathsProjections.pdf :exports both
sqdeaths <- deaths
...
#+name: Q3DeathTests
#+caption: Question 3: Visual residuals testing for base model
#+begin_src R :session MyR :results graphics file :file babelTSDiagDeath.pdf :exports both
...
#+name: Q3SQDeathTests
#+caption: Question 3: Visual residuals testing for back-fitted model
#+begin_src R :session MyR :results graphics file :file babelTSDiagSQDeath.pdf :exports both
...
#+name: Q3StatTests
#+caption: Question 3: Stationarity testing
#+begin_src R :session MyR :results output :exports both
...
#+name: Q3HypoTest
#+caption: Question 3: Post-regression residual testing
#+begin_src R :session MyR :results output :exports both
...
* Question 4 Visualisations
:PROPERTIES:
:CUSTOM_ID: app:Q4Viz
:END:

#+name: Q4RetViz
#+caption: Question 4: Logged returns visualised
#+begin_src R :session MyR :results graphics file :file babelEXRetTSPLT.pdf :exports both
...
#+name: Q4ARIMARes
#+caption: Visual residuals test for ARIMA model
#+begin_src R :session MyR :results graphics file :file babelTSDAGExPostEPlts.pdf :exports both
  ts.diag(arima_mod_post)
#+end_src

#+caption: Visual residuals test for ARIMA model
#+RESULTS: Q4ARIMARes
[[file:babelTSDAGExPostEPlts.pdf]]
...
#+name: Q4StatTest
#+caption: Question 4: Stationarity testing
#+begin_src R :session MyR :results output :exports both
...
#+name: Q4ModelFit
#+caption: Question 4: ARIMA mean model fitting
#+begin_src R :session MyR :results output :exports both
...
#+name: Q4GARCHFit
#+caption: Question 4: GARCH Model Fitting
#+begin_src R :session MyR :results output :exports both
